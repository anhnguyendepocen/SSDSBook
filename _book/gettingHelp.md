...
With the three functions written in a `.R` script, highligh all seven lines of code (the three functions, both comments, and both whitespace lines)  and copy them to your clipboard (`Edit > Copy`). Then call the `reprex()` function. Markdown formatted text that weaves both the functions and their output together will appear in the viewer tab:
...
### External Venues
There are two good places to ask questions external to the course as well. For questions related to RStudio, R Markdown, and the [tidyverse](http://tidyverse.org), RStudio maintains a web forumn called [RStudio Community](https://community.rstudio.com). If you are posting there, you can use `reprex()` to generate Markdown formatted reprexs.
...
Of the two sites, I find [RStudio Community](https://community.rstudio.com) to be a friendlier place that is more relaxed thatn Stack Overflow and its cousins. However, Stack Exchange communities have much larger user bases to draw from.
