...
```{block, type = "rmdnote"}
A cloned directory is one that retains a digital link to the data stored on GitHub, meaning that it can be easily updated if changes are made. This will be explained in greater depth in Introduction to GitHub chapter in this text. 
```
...
```{block, type = "rmdwarning"}
This directory is only applicable to students in [Introduction to GIS](https://slu-soc5650.github.io), who will receive a data release at the beginning of the semester that should be saved here. Students in [Quantitative Analysis](https://slu-soc5050.github.io) will receive most data through `R` packages and may reiceve a small amount of data through weekly repositories.
```
...
```{block, type = "rmdwarning"}
Unlike `Core-Documents`, you have the ability to push changes made here to GitHub.com. Please make sure you understand that changes made will be reflected, for better or worse, on the website the next time you complete that push.
```
...
```{block, type = "rmdnote"}
You will want to tailor this subdirectory's contents to match the specific requirements of the section you are enrolled in. This may entail removing certain folders - the `AnnotatedBib/` for example - that do not correspond to deliverables you have to complete.
```
...
```{block, type = "rmdwarning"}
You will not be able to push changes that you make in these directories to GitHub, and making other changes could cause conflicts. I suggest not making changes inside this repository beyond opening files for reference purposes. If there is something you need to edit, copy it to the appropriate subdirectory (`Labs`, `WeeklyPreps`, etc.) and make local changes there.
```
...
```{block, type = "rmdtip"}
macOS users should make sure that the external drive is formatted using the ExFat file system specification so that it is readable both by their operating system and the Windows operating system in the computer lab. If you buy a new drive, you will likely have to re-format it.
```
